...
export function categoryToURL(
  categoryId: string,
  fromIndex: number,
  sortBy: '' | 'price' | 'rating',
  reverse: boolean,
): string {
  return `${SERVER_URL}/goods/category/${categoryId}?start=${fromIndex}&count=11&sortBy=${sortBy}&reverse=${reverse}`;
}
...
export function subCategoryToURL(
  ...
  return `${SERVER_URL}/goods/category/${categoryId}/${subCategoryId}?start=${fromIndex}&count=11&sortBy=${sortBy}&reverse=${reverse}`;
}
